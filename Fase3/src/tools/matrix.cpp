...
void getCatmullRomPoint(float t, const std::vector<float>& p0, const std::vector<float>& p1,
    ...
    // Itera sobre cada dimensão (x, y, z)
    for (int i = 0; i < 3; ++i) {
        // Calcula a posição do ponto na curva usando a fórmula do spline Catmull-Rom
        pos[i] = 0.5f * ((2 * p1[i]) +
            (-p0[i] + p2[i]) * t +
            (2 * p0[i] - 5 * p1[i] + 4 * p2[i] - p3[i]) * t2 +
            (-p0[i] + 3 * p1[i] - 3 * p2[i] + p3[i]) * t3);

        // Calcula a derivada no ponto da curva para obter a direção da tangente
...
void getGlobalCatmullRomPoint(float gt, std::vector<std::vector<float>> controlPoints, float* pos, float* deriv) {
    // Obtém o número de pontos de controle
    size_t POINT_COUNT = controlPoints.size();

    // Converte o tempo global em uma fração do segmento entre dois pontos de controle
    float t = gt * POINT_COUNT;

    // Determina o índice do ponto de controle de início
    int index = floor(t);

    // Calcula a posição fracionária dentro do segmento
    t = t - index;

    // Determina os quatro índices de pontos de controle usados para o cálculo
    int indices[4];
    ...
    // Chama a função getCatmullRomPoint para calcular a posição e a derivada
