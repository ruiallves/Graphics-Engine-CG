...
int createFileType(Figura figura, string name, int arg1, int arg2, int arg3, int arg4) {
    ...
    // Grava as coordenadas dos vértices
    LinkedList currentVertice = getFiguraVertices(figura);
    while (currentVertice != nullptr) {
        Vertice v = (Vertice)getData(currentVertice);
        if (v != nullptr) {
            file << getVerticeX(v) << "," << getVerticeY(v) << "," << getVerticeZ(v);
        }

        // Verifica se existem normais e textcoords associados aos vértices
        LinkedList currentNormal = getFiguraNormais(figura);
        LinkedList currentTextCoord = getFiguraTextCoordenadas(figura);

        // Grava as normais se estiverem presentes
        if (currentNormal != nullptr) {
            Vertice normal = (Vertice)getData(currentNormal);
            file << "," << getVerticeX(normal) << "," << getVerticeY(normal) << "," << getVerticeZ(normal);
            currentNormal = (LinkedList)getNext(currentNormal);
        }

        // Grava as textcoords se estiverem presentes
        if (currentTextCoord != nullptr) {
            Vertice textCoord = (Vertice)getData(currentTextCoord);
            file << "," << getVerticeX(textCoord) << "," << getVerticeY(textCoord);
            currentTextCoord = (LinkedList)getNext(currentTextCoord);
        }
...
int main(int argc, char** argv) {
    ...
    case PLANE:
        ...
        createFileType(figura, argv[4], std::stof(argv[2]), std::stof(argv[3]), -1, -1);
    ...
    case SPHERE:
        ...
        createFileType(figura, argv[5], std::stof(argv[2]), std::stof(argv[3]), std::stof(argv[4]), -1);
        break;
    ...
    case CONE:
        ...
        createFileType(figura, argv[6], std::stof(argv[2]), std::stof(argv[3]), std::stof(argv[4]), std::stof(argv[5]));
        break;
    ...
    case SURFACE:
        ...
        createFileType(figura, argv[4], std::stof(argv[3]), -1, -1, -1);
        break;
