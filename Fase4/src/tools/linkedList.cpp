...
vector<float>* linkedListToVectorFloat(LinkedList head) {
	std::vector<float>* result = new std::vector<float>();

	// Percorre a linked list
	while (head != nullptr) {
		// Converte o ponteiro de dados para float e adiciona ao vetor
		float* floatPtr = static_cast<float*>(head->data);
		result->push_back(*floatPtr);

		// Avança para o próximo nó da linked list
		head = head->next;
	}
